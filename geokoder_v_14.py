...
def geocode(kod, miasto, ulica, numer, numer_linii_pliku):
    ...
    print(str(numer_linii_pliku) + "  " + service + paramsUrl)
...
def pisz_do_pliku(kod, miasto, ulica, numer, numer_linii_pliku, plik1, plik2, plik3, plik4, id_adres, wiersz):
    miasto, ulica, numer, kod = str_with_nan_all(miasto, ulica, numer, kod)
    geomWkt, x, y, how_many_results = geocode(kod, miasto, ulica, numer, numer_linii_pliku)
    # Sprawdzamy czy może wynik 0 nie jest spwodowany niepoprawnym kodem pocztowym w  zestawieniu z nazwą miasta
    if how_many_results == 0:
        how_many_results = -100
        geomWkt, x, y, how_many_results = geocode('', miasto, ulica, numer, numer_linii_pliku)
    if how_many_results == 0:
        plik1.write(
            id_adres + ";" + str(numer_linii_pliku) + ";" + kod + ";" + miasto + ";" + ulica + ";" + numer + "\n")
    elif how_many_results == 1:
        #plik2.write(
            #id_adres + ";" + str(numer_linii_pliku) + ";" + kod + ";" + miasto + ";" + ulica + ";" + numer + ";" + str(
                #x) + ";" + str(y) + ";" + geomWkt + "\n")
        tekst=""
        ...
        plik4.write(
            id_adres + ";" + str(numer_linii_pliku) + ";" + kod + ";" + miasto + ";" + ulica + ";" + numer + "\n")
    elif how_many_results == -100:
        print('Wystapil blad w algorytmie. Niemożliwa wartość ow_many_results == -100')
...
def otworz_pliki():
    try:
        ...
        #jeden.write("ID_adres;numer_linii_pliku;kod;miasto;ulica;numer_domu;x;y;geomWkt\n")
...
if __name__ == '__main__':
    df = pd.read_csv(file_in, delimiter=';', header=[0], encoding='utf-8')  # wczytywanie danych
    df = df.reset_index()
    ...
    for index, row in generator:
        ...
        while paused:
            if keyboard.is_pressed('a'):  # po wcisnieciu a wznawia program
                zero, jeden, wiecej, blad = otworz_pliki()
                paused = False
        pisz_do_pliku(kod=row[nazwy_kolumn['kod']], miasto=row[nazwy_kolumn['miasto']],
                      ulica=row[nazwy_kolumn['ulica']], numer=row[nazwy_kolumn['numer']], numer_linii_pliku=index + 1,
                      plik1=zero, plik2=jeden, plik3=wiecej, plik4=blad,
                      id_adres=str(row['Nr OKW']) + "_" + str(row['TERYT gminy']) + "_" + str(row['Numer']),
                      wiersz = row)
    zamknij_pliki(zero, jeden, wiecej, blad)